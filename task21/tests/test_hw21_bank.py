...
def test_open_deposit_account_success(bank, client_data):
    ...
    result = bank.open_deposit_account(client_data["client_id"], client_data["start_balance"], client_data["years"])
...
def test_open_deposit_account_fail_unregistered(bank, client_data):
    result = bank.open_deposit_account(999, client_data["start_balance"], client_data["years"])
...
def test_interest_formula(bank, client_data):
    bank.register_client(client_data["client_id"], client_data["client_name"])
    bank.open_deposit_account(client_data["client_id"], client_data["start_balance"], client_data["years"])
    final_balance = bank.calc_deposit_interest_rate(client_data["client_id"])
    expected = round(client_data["start_balance"] * (1 + 0.1 / 12) ** (12 * client_data["years"]), 2)
